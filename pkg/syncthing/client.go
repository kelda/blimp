...
func (c Client) Run(scc sandbox.ControllerClient, finishedInitialSync <-chan struct{}) ([]byte, error) {
	...
	go c.syncFileHash(scc, finishedInitialSync)
...
func (c Client) syncFileHash(scc sandbox.ControllerClient, stop <-chan struct{}) {
	...
	sendOnce := func() {
		...
		_, err := scc.UpdateVolumeHashes(context.Background(), &sandbox.UpdateVolumeHashesRequest{
			Hashes: hashes,
		})
		if err != nil {
