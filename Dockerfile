...
RUN CGO_ENABLED=0 go install -i -mod=vendor -ldflags "${COMPILE_FLAGS}" ./pkg/...

ADD ./registry ./registry
RUN CGO_ENABLED=0 go install -i -mod=vendor -ldflags "${COMPILE_FLAGS}" ./registry/...

ADD ./sandbox ./sandbox
RUN CGO_ENABLED=0 go install -i -mod=vendor -ldflags "${COMPILE_FLAGS}" ./sandbox/...

ADD ./cluster-controller ./cluster-controller
RUN CGO_ENABLED=0 go install -i -mod=vendor -ldflags "${COMPILE_FLAGS}" ./cluster-controller/...
